...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet => google.exists(g => tweet.text.contains(g)))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => apple.exists(g => tweet.text.contains(g)))
